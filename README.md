...
## pi bridge: a self-describing, MCP/webhook-capable coding agent

[`pi_bridge/`](pi_bridge/) vendors a [pi](https://github.com/earendil-works/pi)
coding-agent extension that gives a running `pi` instance the same kind
of self-describing identity a FASP peer already has: a signed,
hash-chained [Agent ID Card](https://github.com/Jayasuryamahadevan/agent-id-card)
per workspace, generated by actually being curious about its
surroundings on first run rather than a static credential. On top of
that, the bridge lets `pi` connect itself to any MCP server (its tools
become `pi` tools automatically, and each one gained is recorded against
the card) and any webhook, in both directions. See
[`pi_bridge/README.md`](pi_bridge/README.md) for the quick start and
[`pi_bridge/extension/`](pi_bridge/extension/) for the implementation --
this is a separate TypeScript component (Node's built-ins plus the
official MCP SDK, no Python), documented on its own.
